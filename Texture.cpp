...
// 当szPathName 是完全路径时，需要指定bUseFullPath为TRUE
BOOL BuildTexture(CString szPathName, GLuint &texid, BOOL bUseFullPath)
{
	HDC       hdcTemp;                                     // DC用来保存位图
	HBITMAP  hbmpTemp;                                     // 保存临时位图
	IPicture *pPicture;                                    // 定义IPicture Interface
	LPTSTR  szPath = new TCHAR[MAX_PATH+1];                // 图片的完全路径
	long  lWidth;                                          // 单位宽度
	long  lHeight;                                         // 单位高度
	...
	if (!bUseFullPath)
	{
		if (szPathName.Left(7) == "http://")               // 如果路径包含 http:// 则...
		{
			lstrcpy(szPath, (LPTSTR)(LPCTSTR)szPathName);  // 把路径拷贝到 szPath
		}
		else                                               // 否则从文件导入图片
		{
			GetCurrentDirectory(MAX_PATH, szPath);         // 取得当前路径
			lstrcat(szPath, (LPTSTR)"\\");                 // 添加字符"\"
			lstrcat(szPath, (LPTSTR)(LPCTSTR)szPathName);  // 添加图片的相对路径
		}
	}
	else
	{
		lstrcpy(szPath, (LPTSTR)(LPCTSTR)szPathName);
	}
	...
	if(FAILED(hr))
	{
		MessageBox (HWND_DESKTOP, L"图片导入失败!", L"出错", MB_OK | MB_ICONEXCLAMATION);
		return FALSE;
	}
